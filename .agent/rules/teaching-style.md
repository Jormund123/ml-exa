...
### Formula-With-Reasoning, Always

This is a **written exam**. What I write on paper is all that matters. But I learn formulas best when I understand **why each step follows from the last**.

For every important topic:

1. **Give a 1-2 sentence intuitive explanation** — what is this thing, in plain language? Then immediately follow with the formula. Never leave it at just intuition.
2. **State the formula** — in the prof's exact notation
3. **Explain what each symbol means** — one line per symbol. **Define every new symbol on first use** with its meaning and dimensions (e.g., "where $A = \Phi\Phi^\top \in \mathbb{R}^{M \times M}$ is symmetric").
4. **Show how to derive it** — using the meta-pattern steps, with **connecting prose between every step** explaining what rule or trick is being applied and why
5. **Show what the exam answer looks like** — as I would write it on paper

Bare formula-to-formula sequences with no text in between are **forbidden**. Every formula must be preceded by 1-2 sentences explaining what we are about to do.

If I can't write it down, I can't score. But if I don't understand why, I can't reconstruct it under pressure.
...
3. **How do you derive it?** (meta-pattern steps, with prose explaining each step)
...
### Show Intermediate Algebra Explicitly (Non-Negotiable)

Never skip algebraic steps. If a derivation line combines two operations, **split them into separate lines**. Specific rules:

- When expanding a product like $(a - b)^\top(a - b)$, show **each of the four terms** individually before combining
- When applying a transpose rule like $(AB)^\top = B^\top A^\top$, **state the rule**, then apply it
- When a sum collapses (e.g., summing a constant $n$ times gives $n \cdot \text{constant}$), **show that step explicitly**
- When using a matrix calculus rule (e.g., $\nabla_w(w^\top A w) = 2Aw$), **name the rule** before applying it

Assume I need to see every single manipulation. If in doubt, show the intermediate step.
...
## WRITE TO NOTES FILES, NOT CHAT (Non-Negotiable)

When explaining formulas or derivations, **always write them directly into the relevant notes file** (in `notes/`) rather than explaining in chat. LaTeX does not render properly in the chat window.

- **Chat** should contain only brief summaries, questions, or pointers to what was added.
- **All substantive math** — formulas, derivations, expansions, clarifications — goes into the markdown notes files where LaTeX renders.
- If I ask about a specific line in a notes file, add the explanation directly into the file at that location.
